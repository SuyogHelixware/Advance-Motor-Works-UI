...
export default function BusinessPartner() {
  ...
  const customerOptions = [
    {
      ...
      path: "/dashboard/Sales/Sales-quotation",
    },
    { label: "Create Sales Order", path: "/dashboard/Sales/Sales-order" },
    { label: "Create A/R Invoice", path: "/dashboard/Sales/Sales-invoice" },
    {
      label: "Create A/R Credit Note",
      path: "/dashboard/Sales/Sales-credit-note",
    },
  ];
  ...
  const vendorOptions = [
    ...
    {
      label: "Create A/P Invoice",
      path: "/dashboard/Purchase/purchase-invoice",
    },
    {
      label: "Create A/P Credit Note",
  ...
  const fetchGetListData = async (
    ...
    try {
      ...
      const url = cleanedSearchTerm
        ? `/BPV2/V2/ByCardType/Search/${cleanedSearchTerm}/${cardType}/1/${pageNum}/20`
        : `/BPV2/V2/ByCardType/Pages/${cardType}/1/${pageNum}/20`;
  ...
  const ClearForm = async () => {
    ...
    setFormBPValue("Series", firstSeries?.SeriesId ?? "");
    setFormBPValue("CardCode", firstSeries?.DocNum ?? "");
    // setFormBPValue(
    //   "GroupCode",
    //   customergroup.length > 0 ? customergroup[0].DocEntry : ""
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      ...
      const url = cleanedSearchTerm
        ? `/BPV2/V2/Search/${cleanedSearchTerm}/1/${pageNum}/20`
        : `/BPV2/V2/Pages/1/${pageNum}/20`;
  ...
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      ...
      const url = cleanedSearchTerm
        ? `/BPV2/V2/Search/${cleanedSearchTerm}/0/${pageNum}/20`
        : `/BPV2/V2/Pages/0/${pageNum}/20`;
  ...
  const SeriesData = async () => {
    try {
      ...
      if (response.success === true) {
        ...
        if (values.length > 0) {
          const seriesId = values[0]?.SeriesId ?? "";
          const docNum = values[0]?.DocNum ?? "";

          console.log("Setting Series:", seriesId);
          console.log("Setting CardCode:", docNum);

          // Reset first to force update
          setFormBPValue("Series", "");
          setFormBPValue("CardCode", "");

          setTimeout(() => {
            setFormBPValue("Series", seriesId, { shouldValidate: true });
            setFormBPValue("CardCode", docNum, { shouldValidate: true });
          }, 0);
        }
      } else {
        Swal.fire({
  ...
  const handleSubmitFormBP = async (data) => {
    ...
    if (saveUpdateNameBP === "SAVE") {
      try {
        const response = await apiClient.post(`/BPV2/V2`, BussPatner);
              ...
              <Box sx={{ display: "flex", gap: 1 }}>
                {saveUpdateNameBP === "UPDATE" && (
                  <>
                    ...
                    >
                      {(cardType === "C" ? customerOptions : vendorOptions).map(
                        (item) => (
                          <MenuItem
                            key={item.label}
                            onClick={() => handleCreateDoc(item.path)}
                          >
                            {item.label}
                          </MenuItem>
                        ),
