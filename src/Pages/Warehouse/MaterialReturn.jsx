...
export default function IssueMaterial() {
  ...
  const handleBarcodeChange = (event) => {
    ...
    const updatedItems = oLines.map((item) => {
      ...
      if (itemCode === enteredCode) {
        ...
        return {
          ...item,
          ReturnQuantity: (currentReturnQty + 1).toFixed(3),
        };
      }
    ...
    if (!itemFound) {
      Swal.fire({
        text: "Item not found in the list",
        icon: "warning",
        toast: true,
        showConfirmButton: false,
        timer: 2000,
  ...
  const getHW_WMSStaffList = (defaultValue = null) => {
    apiClient
      .get(`/Technician?HW_WMSStaff=Y`)
      .then((response) => {
        setWMSStaff(response.data.values);
        // After list loads, set the saved value so the dropdown shows the correct selection
        if (
          defaultValue !== null &&
          defaultValue !== undefined &&
          defaultValue !== ""
        ) {
          setValue("HW_WMSStaff", defaultValue);
        }
      })
      .catch((error) => {
        console.log(error);
  ...
  const handleSubmitForm = async (data) => {
    ...
    if (!data.RequestNo) {
      Swal.fire({
        text: "Please select Issue Material...",
        icon: "warning",
        toast: true,
        showConfirmButton: false,
        timer: 2000,
    ...
    // Check zero return qty — from oLines STATE
    const zeroReturnQty = oLines.filter(
      (line) => !line.ReturnQuantity || Number(line.ReturnQuantity) === 0,
    ).length;
    ...
    // Build payload using oLines STATE (not data.oLines which is always stale)
    const payload = {
      UserId: UserId,
      CreatedBy: CreatedBy,
      ReturnNo: data.ReturnNo,
  ...
  const onSelectRequest = (selectedItem) => {
    ...
    setoLines(mappedOLines);
    setValue("oLines", mappedOLines);
    // Load WMS staff list then set the saved HW_WMSStaff value once list is ready
    getHW_WMSStaffList(filledValues.HW_WMSStaff);
    setIsDialogOpen(false);
  };
