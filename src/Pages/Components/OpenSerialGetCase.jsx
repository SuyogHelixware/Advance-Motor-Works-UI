...
const SerialOuttake = ({
  ...
  useEffect(() => {
    if (openserialmodal) {
      ...
      // setSelectedSeries(oSerialLines.length)
      setItemCache({});
    }
  ...
  const handleSubmit = () => {
    ...
    const selectedData = generatedData.filter((row) =>
      selectedRows1.includes(row.DocEntry),
    );
...
console.log("oSerialLines",oSerialLines)
  ...
  const handleSelected=(ids)=>{
    setSelectedRows1(ids)
   setSelectedSeries(oSerialLines.length + ids.length);
  }
  const progress = rowCount > 0 ? (SelectedSeries / rowCount) * 100 : 0;
  return (
    <>
      <GradientDialog
  ...
  !oSerialLines.some(
    (obj) => obj.IntrSerial === params.row.IntrSerial
  )
}
                checkboxSelection
                onRowSelectionModelChange={(ids) => handleSelected(ids)}
                disableColumnFilter
        ...
        >
          ...
          <Box sx={{ flex: 1 }} />
          <ActionButton variant="outlined" size="small" onClick={DialogClosePayto}>
            Close
          </ActionButton>
