...
const processRowUpdate = (newRow, oldRow) => {
  ...
  const updatedRow = {
    ...oldRow,
    ...newRow,
    Qty: qty, // ensure stored as number
  };

  setData2((prevData) =>
    prevData.map((row) =>
      row.LineNum === updatedRow.LineNum ? updatedRow : row
    )
  );
    ...
    async (page = 0, search = "",ItemCode,WHSCode) => {
      ...
      try {
        ...
        if (data.success) {
          ...
          //   [cacheKey]: items,
          // }));
        let UpdatedQty=items.map((item)=>({...item,Qty:item.Quantity}))
          setData2(UpdatedQty);
  ...
  }, [currentPage, pageSize, searchText, fetchItems, openBatchmodal]);
          ...
          <Box sx={{ display: "flex", gap: 2, mt: 1 }}>
            ...
            >
              <DataGrid
                ...
                checkboxSelection
                rowCount={generatedData.length || 0}
...
export default BatchOutCopyform;
