...
const ToBinLocation = ({
  ...
  const onSubmitBinLoc = () => {
    console.log("Child submitting bins:", rows);
  ...
  setValue("WHSCode", data.ToWHSCode);
  ...
  useEffect(() => {
    if (open && data && typeof data === "object") {
      const filteredBins =
        data.oDocBinLocationLines?.filter(
          (item) => item.BinActionType === "T"
        ) || [];
      setRows(
        filteredBins.length > 0
          ? filteredBins.map((item) => ({ ...item, DocEntry: item.BinAbs }))
          : [
              {
...
export default React.memo(ToBinLocation);
