...
export default function Home() {
  ...
  const [openSOPages, setOpenSOPages] = useState({});
  ...
  useEffect(() => {
    ...
    const parentMenu = parsed.SubMenus?.find((menu) => menu.ParentMenuId === 1);
    setDashboardMenus(parentMenu?.Menus || []);
  }, []);

  const hasPermission = (menuId) =>
    dashboardMenus.some((menu) => menu.MenuId === menuId && menu.IsRead);

  const SALES_MANAGER = hasPermission(376);
  const Sales_DashBoard = hasPermission(377);
  const WORKSHOP_DASHBOARD = hasPermission(378);
  const WAREHOUSE_DASHBOARD = hasPermission(379);
  ...
  const getAllSalesAnalysis = async (filterDate = null) => {
    ...
    try {
      const endpoint = `/DashBoard/SalesReports/SalesAnalysisMTD/${filterDate}`;
  ...
  const getAllSalesServiceList = async (filterDate = null) => {
    ...
    try {
      const endpoint = `/DashBoard/Workshop/SalesMTDService/${filterDate}`;
      const res = await apiClient.get(endpoint);
      const values = res.data.values || [];
  ...
  const getAllTargetVsActual = useCallback(async () => {
    ...
    try {
      ...
      const endpoint = `/DashBoard/SalesReports/TargetVSActual/${dayjs(
        selectedMonth,
      ).format("YYYY-MM-DD")}`;
      const res = await apiClient.get(endpoint);
  ...
  const getAllSalesPerson = useCallback(async () => {
    ...
    try {
      ...
      const endpoint = `/DashBoard/SalesReports/TopSalesPersonMTD/${dayjs(
        selectedMonth,
      ).format("YYYY-MM-DD")}`;
  ...
  const getAllTopProducts = useCallback(() => {
    ...
    apiClient
      .get(
        `/DashBoard/SalesReports/Top20ProductsMTD/${dayjs(selectedMonth).format(
          "YYYY-MM-DD",
        )}`,
      )
      .then((res) => {
  ...
  const getAllTopCustomer = useCallback(() => {
    ...
    apiClient
      .get(
        `/DashBoard/SalesReports/Top20BPMTD/${dayjs(selectedMonth).format(
          "YYYY-MM-DD",
        )}`,
      )
      .then((res) => {
  ...
  const getSalesDashOpenSOData = useCallback(async (search = "", page = 0) => {
    ...
    try {
      ...
      const endpoint = search
        ? `/DashBoard/Sales/OpenSO/${user}/${search}/${page}`
        : `/DashBoard/Sales/OpenSO/${user}/${page}`;
  ...
  const getSalesDashTotalMonthsSale = async (filterDate = null) => {
    ...
    try {
      const res = await apiClient.get(
        `/DashBoard/Sales/MonthlySale/${UserId}/${filterDate}`,
      );
  ...
  const getAppointmentData = async (search = "", page = 0) => {
    ...
    try {
      const endpoint = `/DashBoard/Workshop/AptForInward/All`;
  ...
  const getMaterialRequestData = async (search = "", page = 0) => {
    ...
    try {
      ...
      const endpoint = cleanedSearch
        ? `/DashBoard/Warehouse/MatReqForIssue/search/${cleanedSearch}/${page}`
        : `/DashBoard/Warehouse/MatReqForIssue/${page}`;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      await getMaterialRequestData(searchMaterialRequest, materialPage);

      intervalId = setInterval(() => {
        getMaterialRequestData(searchMaterialRequest, materialPage);
      }, 10000);
    };
  ...
  }, [searchMaterialRequest, materialPage]);
      ...
      {SALES_MANAGER && (
        <>
          ...
          <Grid container style={{ paddingRight: 0 }} spacing={2}>
            <Grid item lg={colWidth} md={12} sm={12} xs={12}>
                ...
                <Box p={2}>
                  {loadingSalesDashOpenSO ? (
                    ...
                    <DataGrid
                      ...
                      rowCount={
                        Object.keys(openSOPages).length * 20 +
                        (hasMoreOpenSO ? 20 : 0)
                      }
                      paginationModel={{ page: openSOPage, pageSize: 20 }}
                      onPaginationModelChange={(model) => {
                        const newPage = model.page;
                        setOpenSOPage(newPage);
                        if (openSOPages[newPage]) {
                          setSalesDashOpenSOList(openSOPages[newPage]);
                        } else {
                          getSalesDashOpenSOData(
                            searchSalesDashOpenSO,
                            newPage,
                  ...
                  <ResponsiveContainer width="100%" height={330}>
                    {SalesDashloadingTotalMonthsSale ? (
                      <Box
                        ...
                        height="330px"
                      >
                        <BeatLoader loading={SalesDashloadingTotalMonthsSale} />
                      </Box>
                    ) : (
                      <BarChart
                        data={listSalesDash}
