...
function PurchaseCreditNote() {
  ...
  // const [WhscList, setWhscData] = useState([]);
    ...
    dispatchRedux(fetchExchangeRateStore(docDate))
      ...
      .then((data) => {
        if (SaveUpdateName !== "UPDATE") {
          ...
          const sysRate =
            sysCurr === mainCurr ? 1 : Number(sysRateObj?.Rate || 0);
  ...
  //#region  ClearForm Data

  //#region  Open Tab
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/APCreditNote/Search/${searchTerm}/1/${pageNum}/20`
        : `/APCreditNote/Pages/1/${pageNum}/20`;
  ...
  const fetchCancelledListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/APCreditNote/Search/${searchTerm}/3/${pageNum}/20`
        : `/APCreditNote/Pages/3/${pageNum}/20`;
  ...
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/APCreditNote/Search/${searchTerm}/0/${pageNum}/20`
        : `/APCreditNote/Pages/0/${pageNum}/20`;
  ...
  const onSelectBusinessPartner = async (DocEntry) => {
    ...
    const { values } = dataBP;
    // fetchCopyFromData(values.CardCode);
  ...
  // const fetchCopyFromData = async (
  //   pageNum = 0,
  //   searchTerm = "",
  //   DocType = type,
  // ) => {
  //   let CardCode = getValues("CardCode");
  //   const baseType = watch("baseType") || "18";

  //   try {
  //     const url = searchTerm
  //       ? `/APCreditNote/CopyFrom/Search/${searchTerm}/${CardCode}/${type}/${baseType}/${pageNum}/20`
  //       : `/APCreditNote/CopyFrom/${CardCode}/${type}/${baseType}/${pageNum}/20`;
  ...
  // const WhscListData = async () => {
  //   try {
  //     const res = await apiClient.get(`/WarehouseV2/Pages/1`);
  //     const response = res.data;

  //     if (response.success === true) {
  //       setWhscData(response.values);
  //     } else if (response.success === false) {
  //       Swal.fire({
  //         title: "Error!",
  //         text: response.message,
      ...
      renderCell: (params) => {
        ...
        return (
          <Grid
            ...
            tabIndex={0}
            onKeyDown={(e) => {
              if (params.row.BaseType === "18") return;
              if (e.key === "Backspace") {
                e.preventDefault();
          ...
          >
            ...
            <Grid item sx={{ width: 28, textAlign: "center" }}>
              <IconButton
                size="small"
                disabled={params.row.BaseType === "18"}
                onClick={() => {
                  setValue("selectedRowIndex", rowIndex);
                  dispatch({ type: "OPEN", modal: "TaxFrieghtOpen" });
                }}
  ...
  //         disabled={params.row.Status === "0"}
  //         readOnly={
  //           params.row.BaseType === "21" || params.row.BaseType === "18"
  //         }
  //         value={params.value}
  //         onChange={(e) => handleChange(e, params.row)}
  //         InputProps={{
  ...
  const Items = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        ...
        let disabled =
          params.row.Status === "0" ||
          params.row.Quantity !== params.row.OpenQuantity ||   params.row.BaseType === "21";;
        return (
          <Grid
            container // ✅ important
  ...
  const onSubmit = async () => {
    try {
      const currentData = getValues(); // Always use form state
      const updatedLines = [
        ...(currentData.oLines || []),
        ...(selectedRows || []),
      ];
  ...
  const validateAllLines = (lines) => {
    ...
    lines.forEach((line, index) => {
      ...
      // 🔴 BIN VALIDATION
      if (line.BinActivat === "Y"  && line.BaseType!=="21") {
  ...
  const handleSubmitForm = async (data) => {
    ...
    const obj = {
      ...
      ContactCode: String(data.CntctCode || "0"),
      ...
      TaxOnExAp: String(data.TaxOnExAp || "0"),
      DocTotal: data.DocTotal || "0",
      Series: String(data.Series || "0"),
      PaidToDate: String(data.PaidToDate),
      DueAmnt: String(data.DueAmnt),
      ...
      oLines: data.oLines.map((item) => ({
        ...
        StockSum: String(item.StockSum || ""),
        StockSumSc: String(item.StockSumSc || ""),
        StockSumFc:
          getValues("Currency") === companyData.MainCurncy
            ? "0"
    ...
    if (SaveUpdateName === "SAVE") {
      apiClient
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            Swal.fire({
              title: "Success!",
              text: "Purchase Credit Note Saved Successfully",
              icon: "success",
              confirmButtonText: "Ok",
              timer: 1000,
            });
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          apiClient
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                Swal.fire({
                  title: "Success!",
                  text: "Attachment Purchases Credit Note Updated",
                  icon: "success",
                  confirmButtonText: "Ok",
                  timer: 1000,
                });
          ...
          Swal.fire({
            text: "Attachment Purchases Not Updated",
            icon: "info",
            toast: true,
            showConfirmButton: false,
            timer: 1500,
  ...
  const fetchAndSetData = (DocEntry) => {
    ...
    apiClient
      ...
      .then((response) => {
        ...
        setValue("CountryS", item.oTaxExtLinFes[0].CountryS);
  ...
  const handleOnCancelDocument = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/APCreditNote/Cancel/${currentData.DocEntry}`)
          .then((resp) => {
            if (resp.data.success) {
              ...
              Swal.fire({
                text: "Purchase Credit Note Cancel",
                icon: "success",
                toast: true,
                showConfirmButton: false,
                timer: 1000,
        ...
        Swal.fire({
          text: "Purchase Credit Note Not Cancelled",
          icon: "info",
          toast: true,
          showConfirmButton: false,
          timer: 1500,
  ...
  //     if (result.isConfirmed) {
  //       apiClient
  //         .put(`/APCreditNote/Close/${currentData.DocEntry}`)
  //         .then((resp) => {
  //           if (resp.data.success) {
  //             setClosedListData([]);
  //             fetchClosedListData(0);
  //             ClearForm();
  //             Swal.fire({
  //               text: "Purchase Credit Note Close",
  //               icon: "success",
  //               toast: true,
  //               showConfirmButton: false,
  //               timer: 1000,
  ...
  //     } else {
  //       Swal.fire({
  //         text: "Purchase Credit Note Not Close",
  //         icon: "info",
  //         toast: true,
  //         showConfirmButton: false,
  //         timer: 1500,
  ...
  const onClickCopyFrom = useCallback((e, item, index, DocNum) => {
    ...
    if (checked) {
      ...
      //   const newDiscounts = [...prevDiscounts, item.Discount];
      //   return newDiscounts;
      // });
    } else {
      setoExpLines((prev) =>
  ...
  const onSubmitCopyFrom = (data) => {
    ...
    // Ensure oExpLineData has DocEntry in each object
    ...
    selectedRowsPurchase.forEach((item) => {
      ...
      // let LineTotal=item.LineTotal - ( item.LineTotal * (item.Discount || 0))/100
    });
      ...
      <CopyFromSearchModel
        ...
        Input={
          <>
            ...
            <Grid xs={12} sm={12} md={6} lg={6} textAlign={"center"}>
              <Controller
                name="baseType"
                control={control}
                defaultValue={"18"}
                render={({ field, fieldState: { error } }) => (
                  <InputSelectTextField
                    ...
                    data={[
                      { key: "18", value: "A/P Invoice" },
                      { key: "21", value: "Goods Return" },
                      // { key: "204", value: "	A/P Down Payment" },
                    ]}
                    {...field}
                    ...
                    onChange={(e) => {
                      ...
                      // fetchCopyFromData(0); // Trigger your custom function
                      setOlines([]);
                      setoExpLines([]);
                    ...
                    <Controller
                      ...
                      render={({ field, fieldState: { error } }) => (
                        <InputTextSearchButton
                          ...
                          onChange={field.OpenDailog}
                          type="text"
                          {...field}
                          // error={!!error} // Pass error state to the FormComponent if needed
                          // helperText={error ? error.message : null} // Show the validation message

                          error={!!errors.CardCode} // Pass error state to the InputTextSearchButton
                          helperText={
                            errors.CardCode ? errors.CardCode.message : null
                    ...
                    <Controller
                      ...
                      render={({ field, fieldState: { error } }) => (
                        <InputSelectTextField
                          ...
                          disabled={
                            !allFormData.DocEntry === false || isFieldDisabled
                          }
                          {...field}
                          onChange={(e) => {
                    ...
                    <Controller
                      ...
                      rules={{
                        ...
                        validate: (value) => {
                          ...
                          if (!postingDate)
                            return "Please select posting date first";

                          if (
                            value &&
                            dayjs(value).isBefore(dayjs(postingDate), "day")
                          ) {
...
export default React.memo(PurchaseCreditNote);
