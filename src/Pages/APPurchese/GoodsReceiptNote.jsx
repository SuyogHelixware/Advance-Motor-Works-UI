...
function GoodsReceiptNote() {
  ...
  const perms = usePermissions(134);
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/GRPO/Search/${searchTerm}/1/${pageNum}/20`
        : `/GRPO/Pages/1/${pageNum}/20`;
  ...
  const fetchCancelledListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/GRPO/Search/${searchTerm}/3/${pageNum}/20`
        : `/GRPO/Pages/3/${pageNum}/20`;
  ...
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/GRPO/Search/${searchTerm}/0/${pageNum}/20`
        : `/GRPO/Pages/0/${pageNum}/20`;
  ...
  //   try {
  //     const url = searchTerm
  //       ? `/GRPO/CopyFrom/Search/${searchTerm}/${CardCode}/${type}/${baseType}/${pageNum}/20`
  //       : `/GRPO/CopyFrom/${CardCode}/${type}/${baseType}/${pageNum}/20`;
  //     const response = await apiClient.get(url);
  ...
  const taxCategoruFunc = () => {
    ...
    if (PresentTax) {
      ...
      console.log("oTaxLines", updatedData);
  ...
  const handleCellClick = async (ids) => {
    ...
    const selectedRows = itemList
      ...
      .map((data) => {
        ...
        console.log("Price Bef", PriceBefDi);
  ...
  const CalculateRate = useCallback(() => {
    ...
    //         break;
    //       case "S":
    //         TotalSumSy = ValueFormatter(line.LineTotal / SysRate, 6);
    //         LineTotal = ValueFormatter(TotalSumSy * SysRate, 6);
    //         TotalFrgn = ValueFormatter(LineTotal / DocRate, 6);
    //         break;
    //       case "C":
    //         if (currency === companyData.MainCurncy) {
    //           TotalFrgn = ValueFormatter(0);
    ...
    //             currency === companyData.MainCurncy
    //               ? line.LineTotal
    //               : ValueFormatter(line.TotalFrgn * DocRate);
    //           TotalFrgn = LineTotal / DocRate;
    //           TotalSumSy = isSysCurrency
    //             ? ValueFormatter(LineTotal / DocRate)
    //             : ValueFormatter(LineTotal / SysRate);
    //         }
    //         break;
    //       default:
  ...
  //       if (
  //         name === "Quantity" &&
  //         parseFloat(value) * parseFloat(data.NumPerMsr) >
  //           parseFloat(originalQuantity)
  //       ) {
  //         return {
  //           ...data,
  //           Quantity: originalQuantity / updatedData.NumPerMsr,
  //         };
  //       }
  //     }
  ...
  const handleSubmitForm = async (data) => {
    ...
    //  const jsonString = JSON.stringify(data.oLines[0].oSerialLines);

    // // Actual JSON size in bytes
    ...
    // console.log("🗜️ Compressed JSON Size:", compressedSize, "bytes");
    // console.log("🔥 Compression Saved:", percent + "%");

    const isValidLine = ValidationSubmitForm(
      data,
      type,
      warehouseData,
      getValues,
    ...
    const obj = {
      ...
      DocDate: dayjs(data.DocDate).format("YYYY-MM-DD HH:mm:ss"),
    ...
    if (SaveUpdateName === "SAVE") {
      apiClient
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            Swal.fire({
              title: "Success!",
              text: "Goods Receipt PO saved Successfully",
              icon: "success",
              confirmButtonText: "Ok",
              timer: 1000,
            });
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          apiClient
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                Swal.fire({
                  title: "Success!",
                  text: "Attachment Good Reciept Note Updated",
                  icon: "success",
                  confirmButtonText: "Ok",
                  timer: 1000,
                });
          ...
          Swal.fire({
            text: "Attachment Good Reciept Note Not Updated",
            icon: "info",
            toast: true,
            showConfirmButton: false,
            timer: 1500,
  ...
  const handleOnCancelDocument = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient.put(`/GRPO/Cancel/${currentData.DocEntry}`).then((resp) => {
  ...
  const handleOnCloseDocument = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient.put(`/GRPO/Close/${currentData.DocEntry}`).then((resp) => {
          if (resp.data.success) {
            setOpenListData([]);
            fetchOpenListData(0);
            setClosedListData([]);
            ...
            ClearForm();
            Swal.fire({
              text: "Goods Receipt PO Order Close",
              icon: "success",
              toast: true,
              showConfirmButton: false,
              timer: 1000,
  ...
  //     if (result.isConfirmed) {
  //       apiClient
  //         .put(`/GRPO/Cancel/${currentData.DocEntry}`)
  //         .then((resp) => {
  //           if (resp.data.success) {
  //             ClearForm();
  //             setOpenListData([]);
                      ...
                      {tabvalue === 0 && (
                        ...
                        >
                          <DataGrid
                            ...
                            //     ? "disabled-row"
                            //     : "" ||
                            // errorLineIds.includes(params.id) ? "error-row" : ""
                            // }
...
export default React.memo(GoodsReceiptNote);
