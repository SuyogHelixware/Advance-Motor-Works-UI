...
export default function SalesOrder() {
  ...
  const perms = usePermissions(117);
  ...
  const [selectedTax, setselectRowTax] = useState(null);
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/SalesOrder/Search/${searchTerm}/1/${pageNum}/20`
        : `/SalesOrder/Pages/1/${pageNum}/20`;
  ...
  //#region  Close Tab  Sales Order
  // Close Tab Api Binding
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/SalesOrder/Search/${searchTerm}/0/${pageNum}/20`
        : `/SalesOrder/Pages/0/${pageNum}/20`;
  ...
  const fetchCancelledListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/SalesOrder/Search/${searchTerm}/3/${pageNum}/20`
        : `/SalesOrder/Pages/3/${pageNum}/20`;
      const response = await apiClient.get(url);
      if (response.data.success) {
        const newData = response.data.values;
  ...
  // let BaseType = allFormData.oLines.some((base) => base.BaseType === "17");

  //#region  Small Api Binding
  //Calculation API BINDING Logic
  ...
  //   try {
  //     setIsLoading(true);
  //     const res = await apiClient.get(`/SACSetup/Pages/1/`);
  //     const response = res.data;
  //     if (response.success === true) {
  //       setServiCeList(response.values);
  //     } else {
  //       Swal.fire({
  //         title: "Error!",
  //         text: response.message,
  ...
  // const selectedColumns = type === "S" ? ItemColumn : Services;
  const SelectFreight = () => {
    setopenFreight(true);
  };
      ...
      const newSelections = itemList
        ...
        .map((data) => {
          const PriceTerm = data.oLines.find(
            (itempprice) => itempprice.PriceList === getValues("ListNum"),
          );
    ...
    const obj = {
      ...
      oLines: data.oLines.map((item) => {
        ...
        return {
          ...
          Address: String(item.Address || "0"),
          CogsAcct: item.CogsAcct || "0",
          ...
          CogsOcrCod: item.CogsOcrCod || "",
          GPTtlBasPr: item.GPTtlBasPr || "0",
          GrossBuyPr: item.GrossBuyPr || "0",
          LineStatus: String(item.LineStatus || "0"),
    ...
    if (SaveUpdateName === "SAVE") {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          apiClient
            .put(`/SalesOrder/${data.DocEntry}`, obj)
            .then((response) => {
              if (response.data.success) {
                setOpenListData([]);
                setOpenListPage(0);
                ...
                Swal.fire({
                  title: "Success!",
                  text: "Attachment Sales Order Updated",
                  icon: "success",
                  confirmButtonText: "Ok",
                  timer: 1000,
                });
          ...
          Swal.fire({
            text: "Attachment Sales Order Not Updated",
            icon: "info",
            toast: true,
  ...
  //     if (Data.isConfirmed) {
  //       apiClient
  //         .get(`/SalesOrder/${DocEntry}`)
  //         .then((response) => {
  //           const item = response.data.values;

  //           setType(item.DocType);
  //           setDocEntry(DocEntry);
  //           // setMyObject(item.Status);
  //           // setMyObject(response.data.values);

  //           // console.log("ccccccccc", item.Status);

  //           setSaveUpdateName("UPDATE");
  //           setSidebarOpen(false);
  //           ConatctPerson(item.CardCode, item.CntctCode);
        ...
        .then((response) => {
          ...
          reset({
            ...
            oLines: item.oLines.map((element, index) => ({
              ...
              UomCode: String(element.UomCode || "0"),
              ...
              // LineTotal: String(element.TOTAL_LC),
            })),
  ...
  const handleOnCancel = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/SalesOrder/Cancel/${currentData.DocEntry}`)
          .then((resp) => {
  ...
  const handleOnDocClose = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/SalesOrder/Close/${currentData.DocEntry}`)
          .then((resp) => {
            if (resp.data.success) {
              ClearForm();
              setOpenListData([]);
                      ...
                      render={({ field, fieldState: { error } }) => (
                        <InputSelectTextField
                          ...
                          onChange={(e) => {
                            ...
                            if (selectedSeries !== "0") {
                              const seriesData = slesSeries.find(
                                (item) => item.DocEntry === selectedSeries
                              );
                              if (seriesData && seriesData.NextNumber) {
                                // Set the DocNum field value
                                setValue("DocNum", seriesData.NextNumber);
                              }
                            }
                          }}
                          error={!!error}
