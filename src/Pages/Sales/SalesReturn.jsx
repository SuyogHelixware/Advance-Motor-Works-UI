...
export default function SalesReturn() {
  ...
  const perms = usePermissions(119);
  ...
  // ==============================Data Grid End Pagination And Searching=============================================================
  // =========================TAX State============================
  ...
  // const handleCopyToClick = (event) => {
  //   setAnchorEl(event.currentTarget);
  //   setOpenMenuCopyto(true);
  // };
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/SalesReturn/Search/${searchTerm}/1/${pageNum}/20`
        : `/SalesReturn/Pages/1/${pageNum}/20`;
  ...
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/SalesReturn/Search/${searchTerm}/0/${pageNum}/20`
        : `/SalesReturn/Pages/0/${pageNum}/20`;
  ...
  const fetchCancelledListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/SalesReturn/Search/${searchTerm}/3/${pageNum}/20`
        : `/SalesReturn/Pages/3/${pageNum}/20`;
  ...
  // let BaseType = allFormData.oLines.some((base) => base.BaseType === "16");
  ...
  //* ========================================Small TextField And All Binding========================================
  ...
  const TaxCodeFrieght = async () => {
    ...
    setTaxFrieght(response);
  };

  const CurrencyData = async () => {
    const res = await apiClient.get(`/Currency/pages/0`);
  ...
  const salesEmpdata = async () => {
    const res = await apiClient.get(`/SalesEmp/pages`);
    const response = res.data.values;
    setSalesEmp(response);
  };
  ...
  //   const selectedAccount = taxAccountRows.find(
  ...
  //           ? { ...row, PurchTax: selectedAccount.AcctCode }
  //           : row
  //       )
  //     );
  ...
  //       return (
  //         <Controller
  //           name={`TaxCode_${params.row.id}`} // Unique name per row
  //           control={control}
  ...
  //   { field: "BaseDocNum", headerName: "Base Doc", width: 120 },
  ...
  //         updatedData.InvQty = ValueFormatter(value * updatedData.NumPerMs, 6);
  //       }
  //     }
  //     if (name === "PriceBefDi") {
  ...
  const handleSubmitForm = async (data) => {
    ...
    const obj = {
      ...
      oLines: data.oLines.map((item) => {
        ...
        return {
          ...
          BaseDocNum: item.BaseDocNum || "1",
          BaseOpnQty: item.BaseOpnQty || "10",
          CogsOcrCod: item.CogsOcrCod || "123",
    ...
    // console.log("+++++++++++handlesubmitform+++++++++++", obj);
    // return
    if (SaveUpdateName === "SAVE") {
      apiClient
        .post(`/SalesReturn`, obj)
        .then((res) => {
          if (res.data.success) {
            ClearForm();
            setOpenListData([]);
            ...
            Swal.fire({
              title: "Success!",
              text: "Sale Return saved Successfully",
              icon: "success",
              confirmButtonText: "Ok",
              timer: 1000,
            });
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          apiClient
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                Swal.fire({
                  title: "Success!",
                  text: "Attachment Sales Return Updated",
                  icon: "success",
                  confirmButtonText: "Ok",
                  timer: 1000,
                });
          ...
          Swal.fire({
            text: "Attachment Sales Return Not Updated",
            icon: "info",
            toast: true,
            showConfirmButton: false,
            timer: 1500,
  ...
  const onClickCopyFrom = useCallback((e, item, index) => {
    var checked = e.target.checked;
    // console.log("1111", checked);

    setCheckedItems((prevCheckedItems) => ({
      ...prevCheckedItems,
      [index]: checked, // Use index to track individual checkbox states
    }));
  ...
  const handleOnCancel = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/SalesReturn/Cancel/${currentData.DocEntry}`)
          .then((resp) => {
        ...
        Swal.fire({
          text: "Sales Return Not Canceled",
          icon: "info",
          toast: true,
          showConfirmButton: false,
          timer: 1500,
  ...
  const handleOnDocClose = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/SalesReturn/Close/${currentData.DocEntry}`)
          .then((resp) => {
            if (resp.data.success) {
              ClearForm();
              setOpenListData([]);
              ...
              <Controller
                ...
                render={({ field, fieldState: { error } }) => (
                  <InputSelectTextField
                    ...
                    data={[{ key: "15", value: "Sale Delivery" }]}
                    {...field}
