...
export default function SalesDelivery() {
  ...
  const perms = usePermissions(118);
  ...
  // const handleSubmitAddressPayTo = (data) => {
  //   let PayToAddress = [
  //     data.Address,
  //     data.Street,
  //     data.City,
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/Delivery/Search/${searchTerm}/1/${pageNum}/20`
        : `/Delivery/Pages/1/${pageNum}/20`;
  ...
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/Delivery/Search/${searchTerm}/0/${pageNum}/20`
        : `/Delivery/Pages/0/${pageNum}/20`;
  ...
  const fetchCancelledListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/Delivery/Search/${searchTerm}/3/${pageNum}/20`
        : `/Delivery/Pages/3/${pageNum}/20`;
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      let DocRateLine =
        companyData.MainCurncy === Currency ? "1" : recordRateWise.Rate;
  ...
  const processRowUpdate = (newRow, oldRow) => {
    ...
    if (updatedData.ManBtchNum === "Y" && updatedData.oBatchLines?.length > 0) {
      ...
      if (TwoFormatter(totalBatchQty) !== TwoFormatter(updatedData.InvQty)) {
        Swal.fire({
          icon: "warning",
          title: "Batch quantity mismatch",
          text: `Batch total (${TwoFormatter(
            totalBatchQty,
    ...
    if (SaveUpdateName === "SAVE") {
      apiClient
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            Swal.fire({
              title: "Success!",
              text: "Sale Delivery saved Successfully",
              icon: "success",
              confirmButtonText: "Ok",
              timer: 1000,
            });
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          apiClient
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                Swal.fire({
                  title: "Success!",
                  text: "Attachment Sales Delivery Updated",
                  icon: "success",
                  confirmButtonText: "Ok",
                  timer: 1000,
                });
          ...
          Swal.fire({
            text: "Attachment Sales Delivery Not Updated",
            icon: "info",
            toast: true,
            showConfirmButton: false,
            timer: 1500,
  ...
  const handleCellClickSalesSelected = (id) => {
    ...
    if (checked === false) {
      ...
      // console.log("Selected Rows Data", selectrows);
  ...
  const handleCancelledListClear = () => setCancelledListQuery("");

  const fetchMoreCancelledListData = () => {
    // Fetch more cancelled list data and update state
  };
  ...
  let BaseType = allFormData.oLines.some((base) => base.BaseType === "15");
  ...
  const handleOnCancel = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/Delivery/Cancel/${currentData.DocEntry}`)
        ...
        Swal.fire({
          text: "Sales Delivery Not Canceled",
          icon: "info",
          toast: true,
          showConfirmButton: false,
          timer: 1500,
  ...
  const handleOnDocClose = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/Delivery/Close/${currentData.DocEntry}`)
          .then((resp) => {
            if (resp.data.success) {
              ClearForm();
              setOpenListData([]);
  ...
  const onsubmitBatch = (data) => {
    ...
    console.log("===oLines", UpdatedoBatchLines);
  ...
  "
                  ...
                  {/* ------------------------------------------------------------------------------------------------------- */}
                  <Grid container>
                    <Grid item md={6} lg={3} sm={6} xs={12} textAlign={"center"}>
                      <Controller
                        name="SlpCode"
                        rules={{ required: "this field is required" }}
                    ...
                    <Grid item md={6} lg={3} xs={12} sm={6} textAlign={"center"}>
                      <Controller
                        name="Comments"
                        control={control}
                        render={({ field, fieldState: { error } }) => (
