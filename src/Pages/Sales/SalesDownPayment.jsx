...
function ARSalesDownPaymentInvoice() {
  const theme = useTheme();
  const { user, warehouseData, companyData } = useAuth();
  const perms = usePermissions(120);
  const [state, dispatch] = useReducer(reducer, initialState);
  const [apiloading, setapiloading] = useState(false);
  const { CurrencyRateSelect } = useCurrencyRate();
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/ARSalesDownPayment/Search/${searchTerm}/1/${pageNum}/20`
        : `/ARSalesDownPayment/Pages/1/${pageNum}/20`;
  ...
  //   try {
  //     const url = searchTerm
  //       ? `/ARSalesDownPayment/Search/${searchTerm}/3/${pageNum}/20`
  //       : `/ARSalesDownPayment/Pages/3/${pageNum}/20`;
  ...
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/ARSalesDownPayment/Search/${searchTerm}/0/${pageNum}/20`
        : `/ARSalesDownPayment/Pages/0/${pageNum}/20`;
  ...
  //#region Searching CUSTOMER List
  ...
  //   try {
  //     const url = searchTerm
  //       ? `/ARSalesDownPayment/CopyFrom/Search/${searchTerm}/${CardCode}/${type}/${baseType}/${pageNum}/20`
  //       : `/ARSalesDownPayment/CopyFrom/${CardCode}/${type}/${baseType}/${pageNum}/20`;
  //     const response = await apiClient.get(url);
  //     if (response.data.success) {
  //       const newData = response.data.values;
  ...
  // //   fetchPOData(getListPage + 1, getListSearching ? getListquery : "");
  // //   setGetListPage((prev) => prev + 1);
  // // };
 ...
 const CardCode = getValues("CardCode");
  ...
  const handleDeleteRow = (id) => {
    ...
    reset(updatedData);
    calculateDiscountAmt(parseFloat(allFormData.DpmPrcnt)); // Apply discount to line-level items
  };
      ...
      const updatedLines = currentLines.map((line, index) => {
        if (index === currentRowIndex) {
          return {
            ...
            AcctCode: line.GLMethod === "W" ? DpmClear : line?.DpmClear,
  ...
  const processRowUpdate = (newRow, oldRow) => {
    ...
    const originalQuantity = selectedRowsPurchase.map((item) => item.Quantity);
  ...
  console.log("total Bef", totalBefDiscSy);
    ...
    const obj = {
      ...
      oLines: data.oLines.map((item) => {
        ...
        return {
          ...
          Volume: data.Volume || "0",
          Width1: data.Width1 || "0",
          BaseQty: data.BaseQty || "0",
          Height1: data.Height1 || "0",
          Length1: data.Length1 || "0",
          ObjType: data.ObjType || "203",
          VolUnit: data.VolUnit || "0",
          Weight1: data.Weight1 || "0",
          Len1Unit: data.Len1Unit || "0",
          PostTax: String(item.PostTax || "N"),
          Excisable: data.Excisable || "N",
          DiscPrcnt: data.DiscPrcnt || "0",
          BasePrice: data.BasePrice || "0",
          Hght1Unit: data.Hght1Unit || "0",
          Wdth1Unit: data.Wdth1Unit || "0",
          Wght1Unit: data.Wght1Unit || "0",
          BaseOpnQty: data.BaseOpnQty || "0",
          GPTtlBasPr: data.GPTtlBasPr || "0",
          GrossBuyPr: data.GrossBuyPr || "0",
          GrssProfFC: data.GrssProfFC || "0",
          GrssProfit: data.GrssProfit || "0",
          GrssProfSC: data.GrssProfSC || "0",
          OpenCreQty: data.OpenCreQty || "0",
          ShipToDesc: data.ShipToDesc || "0",
          ...
          oTaxLines: (item.oTaxLines || []).map((taxItem) => ({
            ...
            ObjectType: taxItem.ObjectType || "203",
          })),
        };
      }),
      oExpLines: (data.oExpLines || []).map((item) => ({
        ...
        GrsAmount: String(item.GRSAMT || "0"),
    ...
    if (SaveUpdateName === "SAVE") {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          apiClient
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                Swal.fire({
                  title: "Success!",
                  text: "Attachment Sales Downpayment Updated",
                  icon: "success",
                  confirmButtonText: "Ok",
                  timer: 1000,
                });
          ...
          Swal.fire({
            text: "Attachment Sales DownPayment Not Updated",
            icon: "info",
            toast: true,
  ...
  //     if (result.isConfirmed) {
  //       apiClient
  //         .put(`/ARSalesDownPayment/Cancel/${currentData.DocEntry}`)
  //         .then((resp) => {
  ...
  //     if (result.isConfirmed) {
  //       apiClient
  //         .put(`/ARSalesDownPayment/Close/${currentData.DocEntry}`)
  //         .then((resp) => {
  //           if (resp.data.success) {
  //             setClosedListData([]);
  //             fetchClosedListData(0);
  ...
  const fetchAndSetData = (DocEntry) => {
    ...
    apiClient
      .get(`/ARSalesDownPayment/${DocEntry}`)
      .then((response) => {
  ...
  const onClickCopyFrom = useCallback((e, item, index, DocNum) => {
    ...
    if (checked) {
      ...
      // setValue("DiscPrcnt", item.DiscPrcnt);
  ...
  const handleOpenPurchase = (newrecod, newDataRate) => {
    ...
    const latestDocRate = Number(getValues("DocRate"));
    const sysRate = Number(getValues("SysRate"));
              ...
              {tabData.map(
                ({
                  ...
                  >
                    ...
                    >
                      {data.map((item, i) => {
                        // console.log("OBJ", item); // Log each item individually
                        ...
                        return (
                          <CardComponent
                            ...
                            description={dayjs(item.TaxDate).format(
                              "DD/MM/YYYY",
                            )}
                            searchResult={query}
                            ...
                            onClick={() => {
                              ...
                              console.log("CardCode:", item.CardCode);
                            }}
                          />
                        );
                      })}
                  ...
                  >
                    ...
                    >
                      <Button
                        ...
                        disabled={
                          allFormData.CardCode === "" || allFormData.DocEntry
                        }
                      >
                        COPY FROM
                      </Button>
...
export default React.memo(ARSalesDownPaymentInvoice);
