...
export default function SalesInvoice() {
  ...
  //==========================================================================================================================
  //=====================================get List Copy From Search State====================================================================
  ...
  const DownPaymentColumn = [
    ...
    { field: "VatGroup", headerName: "Tax Code", width: 150 },
    {
      ...
      //     value={params.value}
      //     onChange={(e) => handleOpenDownCals(e, params.row)}
      //     sx={(theme) => ({
  ...
  //       `/ARSalesDownPayment/FromUnsettledDoc/${CardCode}/0/20`,
  //           {
  //        params: {
  ...
  useEffect(() => {
    const rows = DownPaymentRecord.flatMap((item) => [
      ...
      ...(item.oDPMLines || []).map((line) => ({
        id: `line-${item.DocEntry}-${line.LineNum}`,
  ...
  //         const childRatio = value / row.OpenNetAmt;
  //         const Child = childRatio * ChildRows;
  //         const TaxCals = taxCalculation(
  //           Child, // Net amount to draw
  //           0,
  //           0,
  //           0,
  //           0,
  ...
  //         // Only update child if parent validation would pass
  //         if (
  //           name === "NetAmtToDraw" &&
  //           parseFloat(value) > parseFloat(row.OpenNetAmt)
  //         ) {
  //           return r; // Don't update child if parent value would be invalid
  //         }
  //         return {
  //           ...r,
  ...
  //               GrossAmtToDraw: ValueFormatter(totalGross),
  ...
  //         parseFloat(value) > parseFloat(r.OpenNetAmt)
  //       ) {
  //         // Disallow update if new value exceeds the original
  //         return r;
  //       }
  ...
  //         TaxAmtToDraw: ValueFormatter(tax),
  //         GrossAmtToDraw: ValueFormatter(gross),
  //         // OpenNetAmt: ValueFormatter(value),
  //         // OpenTaxAmt: ValueFormatter(tax),
  //         // OpenGrossAmt: ValueFormatter(gross),
  //         oTaxLines: oTaxLines,
  //       };
  //     });
    ...
    if (isParent) {
      ...
      updatedRows = updatedRows.map((r) => {
        ...
        if (r.parentId === newRow.id) {
          ...
          return {
            ...r,
            NetAmtToDraw: ValueFormatter(childNet),
            TaxAmtToDraw: ValueFormatter(tax),
            GrossAmtToDraw: ValueFormatter(childNet + tax),
      ...
      updatedRows = updatedRows.map((r) =>
        r.id === newRow.id
          ? {
              ...
              NetAmtToDraw: value,
              TaxAmtToDraw: ValueFormatter(tax),
              GrossAmtToDraw: ValueFormatter(value + tax),
              oTaxLines: taxCalc?.oTaxLines,
            }
          : r,
      );
  ...
  const onSubmitDownPayment = (data) => {
    ...
    console.log("header data", transformed);
    ...
    const oDPLines = transformed.map((Downheader) => ({
      ...
      VatFc: "0",
      VatSc: "0",
      Gross: Downheader.GrossAmtToDraw,
      GrossFc: "0",
      GrossSc: "0",
      oDPTaxLines: Downheader.oDPMLines.map((childDown) => ({
        ...
        oTaxLines: (childDown.oTaxLines || []).map((taxItem) => ({
          ...
          RelateType: "1",
          RelateEntry: taxItem.RelateEntry || "1",
    ...
    // setDownPostingData(oDPLines);
    const DpmAmnt = oDPLines.reduce((sum, current) => {
      const DpmAmnt = parseFloat(current?.DrawnSum) || 0;
      return sum + DpmAmnt;
    }, 0);
    console.log("DpmAmnt", DpmAmnt);
    setValue("DpmAmnt", DpmAmnt);
    const DpmVat = oDPLines.reduce((sum, current) => {
      const DpmVat = parseFloat(current?.Vat) || 0;
      return sum + DpmVat;
    }, 0);
    setValue("DpmVat", DpmVat);
    console.log("DpmVat", DpmVat);
    ...
    // setValue("oDPLines", oDPLines);
    handleDownPayClose(); // Close modal after save
  };
  // console.log("===========downpayment", allFormData.oDPLines);
      ...
      const updatedLines = currentLines.map((line, index) => {
        if (index === currentRowIndex) {
          return {
            ...
            DftBinAbs,
            BinActivat,
            AcctCode: line.GLMethod === "W" ? RevenuesAc : line?.RevenuesAc,
  ...
  const handleDeleteRow = (id) => {
    ...
    reset(updatedData);
    calculateDiscountAmt(parseFloat(discPercent)); // Apply discount to line-level items
  };
  ...
  const onSubmitDocRate = (data) => {
    setValue("DocRate", data);
  };
  ...
  //               : TotalFrgn;
  //           // const Ratio = DocRateLine / DocRate;
  //           updatedData.LineTotal = ValueFormatter(
  //             updatedData.TotalFrgn * DocRate
  //           );
  //           updatedData.TotalSumSy = ValueFormatter(
  ...
  //     }
  //     // updatedData.Price = CalcLines.discountedPrice;
  //     // updatedData.TotalSumSy = ValueFormatter(CalcLines.LineTotal / SysRate);
  //     // updatedData.TotalFrgn = ValueFormatter(CalcLines.LineTotal / DocRate);
  //     // updatedData.INMPrice = CalcLines.discountedPrice;
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/ARInvoiceV2/Search/${searchTerm}/1/${pageNum}/20`
        : `/ARInvoiceV2/Pages/1/${pageNum}/20`;
      ...
      .then((response) => {
        ...
        reset({
          ...
          oLines: item.oLines.map((element, index) => ({
            ...
            OpenQuantity: String(element.OpenQuantity || "1"),
            LineTotal: String(element.LineTotal),
            IsCommited: String(element.IsCommited),
            AssblValue: element.AssblValue || "0",
            UomEntry2: element.UomEntry2 || "",
            // Line_tax: (element.LineTotal * element.TaxCode) / 100,
          ...
          oExpLines: item.oExpLines.map((element, index) => ({
            ...element,
            UserId: user.userid,
            TaxType: element.TaxType || "0",
            BaseRef: element.DocNum || "-1",
            FixCurr: element.FixCurr || "0",
            Status: element.Status,
  ...
  const fetchClosedListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/ARInvoiceV2/Search/${searchTerm}/0/${pageNum}/20`
        : `/ARInvoiceV2/Pages/0/${pageNum}/20`;
  ...
  const fetchCancelledListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/ARInvoiceV2/Search/${searchTerm}/3/${pageNum}/20`
        : `/ARInvoiceV2/Pages/3/${pageNum}/20`;
  ...
  // let BaseType = allFormData.oLines.some((base) => base.BaseType === "13");
  ...
  const onSelectBusinessPartner = async (DocEntry) => {
    ...
    let cur =
      values.Currency === "AC" ? companyData.MainCurncy : values.Currency;
    // DownPaymentApi(values.CardCode, cur);
  ...
  //   const baseType = watch("baseType") || "17";
  //   try {
  //     const url = searchTerm
  //       ? `/ARInvoiceV2/CopyFrom/Search/${searchTerm}/${CardCode}/${type}/${baseType}/${pageNum}/20`
  //       : `/ARInvoiceV2/CopyFrom/${CardCode}/${type}/${baseType}/${pageNum}/20`;
  ...
  const handleSelectTax = (id) => {
    ...
    const updatedLines = lines.map((line, index) => {
      if (index !== currentRowIndex) return line;
      // Create a new updated object with selected TaxCodec
  ...
  const handleOpenSales = (newrecod, newDataRate) => {
    ...
    if (!result) return; // missing rates → modal already opene
  ...
  const validateAllLines = (lines) => {
    ...
    lines.forEach((line, index) => {
      ...
      // 🔴 BIN VALIDATION
      if (line.BinActivat === "Y" && line.BaseType !== "15") {
  ...
  const handleSubmitForm = async (data) => {
    ...
    const obj = {
      ...
      oDPLines: (data.oDPLines || []).map((odownpayment) => ({
        ...
        ModifiedBy: user.UserName,
        ModifiedDate: dayjs(undefined).format("YYYY-MM-DD "),
        oDPTaxLines: (data.oDPTaxLines || []).map((oDPTax) => ({
          ...oDPTax,
        ...
        //  oTaxLines: odownpayment.oTaxLines.map((taxItem) => ({
        //   ...taxItem,
        //   ModifiedBy: user.UserName,
        //   CreatedDate: dayjs(undefined).format("YYYY-MM-DD"),
        //   ModifiedDate: dayjs(undefined).format("YYYY-MM-DD"),
    ...
    if (SaveUpdateName === "SAVE") {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          apiClient
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                Swal.fire({
                  title: "Success!",
                  text: "Attachment Sales Invoice Updated",
                  icon: "success",
                  confirmButtonText: "Ok",
                  timer: 1000,
                });
          ...
          Swal.fire({
            text: " Attachment Sale Invoice Not Updated",
            icon: "info",
            toast: true,
            showConfirmButton: false,
            timer: 1500,
  ...
  const handleOnCancel = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        apiClient
          .put(`/ARInvoiceV2/Cancel/${currentData.DocEntry}`)
        ...
        Swal.fire({
          text: "Sales Invoice Not Canceled",
          icon: "info",
          toast: true,
          showConfirmButton: false,
          timer: 1500,
  ...
  //     if (result.isConfirmed) {
  //       apiClient
  //         .put(`/ARInvoiceV2/Close/${currentData.DocEntry}`)
  //         .then((resp) => {
  //           if (resp.data.success) {
  //             ClearForm();
  //             setOpenListData([]);
            ...
            >
              <Paper sx={{ width: "100%" }}>
                <DataGrid
                  ...
                  rowHeight={45}
                  rows={copyFromModelSales.map((item, index) => ({
                ...
                <Grid container>
                  ...
                  <Grid item xs={12} sm={6} md={4} lg={4} textAlign={"center"}>
                    <Controller
                      ...
                      // rules={{required:"DocType is Required"}}
                      render={({ field, fieldState: { error } }) => (
                        <InputSelectTextField
                          type="text"
                          {...field}
