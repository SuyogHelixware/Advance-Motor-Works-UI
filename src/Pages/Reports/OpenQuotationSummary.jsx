...
export default function OpenQuotationSummary() {
  const theme = useTheme();
  const gridSx = useMemo(() => dataGridSx(theme), [theme]);
  const [loading, setLoading] = useState(false);
  const [openquotationTableList, setopenquotationTableList] = useState([]);
  ...
  const quotationColumns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <span>
          {params.row.QuotationDate
            ? dayjs(params.row.QuotationDate).format("YYYY-MM-DD")
            : ""}
        </span>
      ),
    },
    {
      ...
      renderCell: (params) => (
        <span>
          {params.row.QuotationDate
            ? dayjs(params.row.QuotationDate).format("hh:mm A")
            : ""}
        </span>
      ),
    },
    {
      ...
      renderCell: (params) => (
        ...
        >
          {params.row.QuotationNo.toUpperCase()}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      width: 80,
      renderCell: (params) =>
        params.row.SpecialDisc + params.row.DesiredDisc || 0,
    },
    {
      field: "TotalDocAmt",
      headerName: "TOTAL",
      ...
      renderCell: (params) => (
        <span style={{ display: "block", textAlign: "right", width: "100%" }}>
          {params.row.TotalDocAmt.toFixed(3) || "0.00"}
        </span>
      ),
    },
    {
      ...
      renderCell: (params) => (
        <span style={{ display: "block", textAlign: "right", width: "100%" }}>
          {params.row.DPPaid.toFixed(3) || "0.00"}
        </span>
      ),
    },
    {
  ...
  const getOpenQutSummary = async (userIds = personName) => {
    setLoading(true);
    try {
      ...
      const res = await apiClient.get(
        `/Reports/OpenQuotationSummary/${from}/${to}?${userIdsParam}`,
      );

      const values = res.data?.values || [];
      setopenquotationTableList(values);
    } catch (err) {
            ...
            >
              ...
              >
                ...
                <Button
                  ...
                  onClick={() => getOpenQutSummary(personName)}
