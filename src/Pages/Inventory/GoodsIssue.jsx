...
import AllBinLocationShow from "../Components/AllBinLocationShow.jsx";
...
import BatchOuttake from "../Components/BatchOuttake.jsx";
import SerialOuttake from "../Components/OpenSerialGetCase.jsx";
...
export default function GoodsIssue() {
  ...
  // console.log("watchSeries:", watchSeries);
  ...
  const fetchOpenListData = async (pageNum, searchTerm = "") => {
    try {
      const url = searchTerm
        ? `/GoodsIssue/Search/${searchTerm}/1/${pageNum}/20`
        : `/GoodsIssue/Pages/1/${pageNum}/20`;
  ...
  //==============================Main Datagrid Related Functions======================================
  ...
  //       (updatedData.Quantity * updatedData.PriceBefDi * Rate) / SysRate;
  ...
  const clearFormData = async () => {
    ...
    await fetchExchangeRateGeneric({
      apiClient,
      Currency: PriceListData[0]?.PrimCurr ?? "",
      DocDate: dayjs(undefined).format("YYYY-MM-DD"),
      companyData: companyData,
      setValue,
      dispatch,
  ...
  const handleOnSubmit = async (data) => {
    ...
    const PostObj = {
      ...
      DocTotalSy: String(data.DocTotalSy),
    ...
    if (SaveUpdateName === "SAVE") {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          apiClient
            .put(`/GoodsIssue/${data.DocEntry}`, PostObj)
            .then((response) => {
              if (response.data.success) {
                clearFormData();
                setopenListPage(0);
  ...
  const onSelectRequest3 = async (
    ...
    const Olines = getValues("oLines");
    const updatedLines = Olines.map((line, index) => {
  ...
  const handleGetListSearch2 = (res) => {
    ...
    timeoutRef.current = setTimeout(() => {
      fetchgetListDataAccount(0, res, httpRequestToken);
    }, TimeDelay);
  };
